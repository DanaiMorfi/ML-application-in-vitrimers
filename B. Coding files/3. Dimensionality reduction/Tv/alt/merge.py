...
draft_path = "normalized_Draft4_cleaned_final2.xlsx"
smiles_path = "normalized_SMILES_list_cleaned_final2.xlsx"
...
smiles_df_unique = (
    ...
    .drop(columns=["SMILES"], errors="ignore")  # Drop SMILES column here
)
...
for col in reversed(target_columns):  # reverse = avoids shifting as we insert
    suffix = col.split()[-1]
    ...
    # Split the DataFrame to insert in place
    insert_at = augmented_df.columns.get_loc(col) + 1
    left = augmented_df.iloc[:, :insert_at]
    right = augmented_df.iloc[:, insert_at:]

    # Join everything together without fragmentation
    augmented_df = pd.concat([left, new_data, right], axis=1)

# === 6. Save to Excel (optional) ===
augmented_df.to_excel("augmented_output_clean12.xlsx", index=False)

print("Augmentation complete! SMILES features added and original SMILES column removed.")
